#!/usr/bin/env python3
"""
regenerate_scenario_4.py
────────────────────────
Regenerates scenario 4 with a guaranteed centered, visible swimmer in calm water.
"""
...
logger = logging.getLogger("RefineScenario4")
...
def main():
    ...
    out_dir = Path("runs/stage3_video_generation_v2/scenario_04_treading_water_coastal_channel")
    ...
    prompt = "(a person swimming breaststroke in clear water:1.5), (visible human swimmer centered in ocean:1.4), top-down aerial drone view of a person swimming in clear calm coastal water, rhythmic arm strokes and kicking, delicate expanding ripples, calm sea, centered swimmer"
    seed = 314

    logger.info("Generating refined scenario 4...")
    ...
    # 2. MP4
    mp4_path = out_dir / "scenario_04_treading_water_coastal_channel.mp4"
    export_video_cv2(frames, mp4_path, fps=8)

    # 3. GIF and WebP
    gif_path = out_dir / "scenario_04_treading_water_coastal_channel.gif"
    webp_path = out_dir / "scenario_04_treading_water_coastal_channel.webp"
    ...
    # 4. Contact sheet
    sheet_path = out_dir / "scenario_04_treading_water_coastal_channel_keyframes.jpg"
    export_contact_sheet(frames, sheet_path)
    logger.info(f"✓ Refined scenario 4 successfully saved to {out_dir}")
